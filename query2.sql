...
CREATE TABLE d_asran_hamgaalagch(h_id int not null, 
                                etsgiin_ner varchar(45),
				etsgiin_utasnii_dugaar int(8),
				ehiin_ner varchar(45),
				ehiin_utasnii_dugaar int(8),
                                CONSTRAINT d_asran_hamgaalagch_fk_h_id FOREIGN KEY (h_id) REFERENCES d_huuhed(id) ON DELETE CASCADE,
                                PRIMARY KEY(h_id),
                                UNIQUE (etsgiin_utasnii_dugaar),
                                UNIQUE (ehiin_utasnii_dugaar));
