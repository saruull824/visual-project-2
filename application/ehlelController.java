...
public class ehlelController implements Initializable{
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		try {
		    ...
		    ResultSet rs = conn.createStatement().executeQuery("SELECT id, ovog, ner from d_ajiltan where tushaal='emch'");
		    int i = 0;
		    while(rs.next()){
		...
		try {
		 	Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/hospital2", "root", "");			
		    ResultSet rs = conn.createStatement().executeQuery("SELECT id, ovog, ner from d_ajiltan where tushaal='suvilagch'");
		    int i = 0;
		    while(rs.next()){
